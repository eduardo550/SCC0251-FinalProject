...
class Steg():
    ...
    def put_binary_value(self, bits, mode): #Put the bits in the image
        for c in bits:
            while mode == "ebe" and self.ebe_gradient() < 50:
                self.next_slot()
    ...
    def read_bit(self, mode): #Read a single bit into the image
        while mode == "ebe" and self.ebe_gradient() < 50:
            self.next_slot()
    ...
    def read_byte(self, mode):
        return self.read_bits(8, mode)
    
    def read_bits(self, nb, mode): #Read the given number of bits
        bits = ""
        for i in range(nb):
            bits += self.read_bit(mode)
        return bits
    ...
    def encode_binary(self, data, mode):
        l = len(data)
        if self.width*self.height*self.nbchannels < l+64:
            raise SteganographyException("Carrier image not big enough to hold all the datas to steganography")
        self.put_binary_value(self.binary_value(l, 64), mode)
        for byte in data:
            byte = byte if isinstance(byte, int) else ord(byte) # Compat py2/py3
            self.put_binary_value(self.byteValue(byte), mode)
        return self.image

    def decode_binary(self, mode):
        l = int(self.read_bits(64, mode), 2)
        output = b""
        for i in range(l):
            output += bytearray([int(self.read_byte(mode),2,)])
        return output
